...
func Diff(config, live *unstructured.Unstructured, opts ...Option) (*DiffResult, error) {
	...
	if err != nil {
		klog.Info(fmt.Sprintf("Failed to get last applied configuration: %v", err))
	} else {
		if orig != nil && config != nil {
			// status is sometimes not null in the orig when applied manual
			...
			klog.Info(fmt.Sprintf("three-way diff calculation failed: %v. Falling back to two-way diff", err))
		}
	}
...
func TwoWayDiff(config, live *unstructured.Unstructured) (*DiffResult, error) {
	if live != nil && config != nil {
		...
		return &DiffResult{Modified: false, NormalizedLive: string(liveData), PredictedLive: "null"}, nil
	} else if config != nil {
		predictedLiveData, err := json.Marshal(config.Object)
		if err != nil {
			return nil, err
		}
		return &DiffResult{Modified: true, NormalizedLive: "null", PredictedLive: string(predictedLiveData)}, nil
	} else {
...
func ThreeWayDiff(orig, config, live *unstructured.Unstructured) (*DiffResult, error) {
	...
	predicatedLiveYaml, err := yaml.JSONToYAML(predictedLiveBytes)
	if err != nil {
		return nil, err
	}
	normalizedLiveYaml, err := yaml.JSONToYAML(liveBytes)
	if err != nil {
		return nil, err
	}
	// 3. compare live and expected live object
	dr := DiffResult{
		PredictedLive:  string(predicatedLiveYaml),
		NormalizedLive: string(normalizedLiveYaml),
...
func NormalizeSecret(un *unstructured.Unstructured) {
	...
	if err != nil {
		klog.Errorf("Failed to convert from unstructured into Secret as %v", err)
		return
	}
	...
	if err != nil {
		klog.Errorf("object unable to covert from secret as %v", err)
		return
	}
	if secret.Data != nil {
		err = unstructured.SetNestedField(un.Object, newObj["data"], "data")
		if err != nil {
			klog.Errorf("failed to set secret.data as %v", err)
			return
		}
	}
}
...
func normalizeEndpoint(un *unstructured.Unstructured) {
	...
	if err != nil {
		klog.Infof("Failed to convert from unstructured into Endpoints as %v", err)
		return
	}
	...
	if err != nil {
		klog.Info(fmt.Sprintf(couldNotMarshalErrMsg, gvk, err))
		return
	}
	un.Object = newObj
}
...
func normalizeRole(un *unstructured.Unstructured, o Options) {
	...
	if o.ignoreAggregatedRoles {
		...
		if ok {
			...
			if !ok {
				klog.Info(fmt.Sprintf("Malformed aggregrationRule in resource '%s', won't modify.", un.GetName()))
			} else {
				un.Object["rules"] = nil
			}
		}
...
func remarshal(obj *unstructured.Unstructured, _ Options) *unstructured.Unstructured {
