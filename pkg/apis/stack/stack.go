...
func IsStack(path string) bool {
	f, err := os.Stat(path)
	f2, err2 := os.Stat(filepath.Join(path, File))
...
func IsStackFile(path string) bool {
	f, err := os.Stat(path)
	return err == nil && !f.IsDir() && f.Mode().IsRegular() && filepath.Base(path) == File
}
...
func ParseStackConfiguration(path string) (*Configuration, error) {
	...
	err := yaml.ParseYamlFromFile(filepath.Join(path, File), &stack)
	if err != nil {
		return nil, err
	}
