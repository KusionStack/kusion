...
func TestValidateConfig(t *testing.T) {
	testcases := []struct {
		...
		{
			name:    "invalid config non-empty current but empty backends",
			success: false,
			config: &v1.Config{
				Backends: &v1.BackendConfigs{
					Current: "dev",
		...
		{
			name:    "invalid config invalid backend name",
			success: false,
			config: &v1.Config{
				Backends: &v1.BackendConfigs{
					Backends: map[string]*v1.BackendConfig{
						"current": {Type: v1.BackendTypeLocal},
					},
				},
			},
		},
		{
			name:    "invalid config empty type but non-empty config",
			success: false,
			config: &v1.Config{
				Backends: &v1.BackendConfigs{
					Backends: map[string]*v1.BackendConfig{
