...
type workloadGenerator struct {
	...
	moduleConfigs map[string]workspaceapi.GenericConfig
}
...
func completeBaseWorkload(base *workload.Base, config workspaceapi.GenericConfig) error {
	replicas, err := workspace.GetIntFieldFromGenericConfig(config, workload.FieldReplicas)
	if err != nil {
		return err
	}
	if replicas == 0 {
	...
	labels, err := workspace.GetStringMapFieldFromGenericConfig(config, workload.FieldLabels)
	if err != nil {
		return err
	}
	if labels != nil {
	...
	annotations, err := workspace.GetStringMapFieldFromGenericConfig(config, workload.FieldAnnotations)
	if err != nil {
		return err
	}
	if annotations != nil {
